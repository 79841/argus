...
export const getProjectDetailStats = async (
  projectName: string,
  dbOverride?: import('better-sqlite3').Database
): Promise<ProjectDetailStats> => {
  const db = dbOverride ?? getDb()
...
export const getProjectDailyCosts = async (
  days: number = 30,
  dbOverride?: import('better-sqlite3').Database
): Promise<ProjectDailyCost[]> => {
  const db = dbOverride ?? getDb()
