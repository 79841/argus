...
vi.mock('@/shared/lib/setup', () => ({
  disconnectAgents: vi.fn(() => [{ agent: 'claude', success: true, action: 'disconnected' }]),
}))

const { POST } = await import('../route')

const mkRequest = (body: unknown) =>
  new NextRequest('http://localhost/api/setup/disconnect', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify(body),
  })

describe('POST /api/setup/disconnect', () => {
  it('유효하지 않은 JSON이면 400을 반환한다', async () => {
    const req = new NextRequest('http://localhost/api/setup/disconnect', {
      method: 'POST',
      body: 'not-json',
    })
  ...
  it('agents가 배열이 아니면 400을 반환한다', async () => {
    const res = await POST(mkRequest({ agents: 'gemini' }) as never)
    expect(res.status).toBe(400)
  })
})
