...
describe('getToolDailyStats', () => {
  it('일별로 집계된 데이터를 반환한다', () => {
    ...
    const oneDayAgoDate = oneDayAgo.slice(0, 10)

    insertToolResult(testDb, 'Bash', 1, 'sess-1', 'p', 'claude', 100, twoDaysAgo)
    insertToolResult(testDb, 'Bash', 0, 'sess-1', 'p', 'claude', 200, twoDaysAgo)
    insertToolResult(testDb, 'Bash', 1, 'sess-2', 'p', 'claude', 100, oneDayAgo)

    const daily = getToolDailyStats('Bash', 7)

    expect(daily.length).toBeGreaterThanOrEqual(2)

    const dayTwo = daily.find((d) => d.date === twoDaysAgoDate)
    expect(dayTwo).toBeDefined()
    expect(dayTwo!.count).toBe(2)
    expect(dayTwo!.success_count).toBe(1)
    expect(dayTwo!.fail_count).toBe(1)

    const dayOne = daily.find((d) => d.date === oneDayAgoDate)
    expect(dayOne).toBeDefined()
    expect(dayOne!.count).toBe(1)
  })
  ...
  it('날짜 오름차순으로 정렬된다', () => {
    insertToolResult(testDb, 'Bash', 1, 'sess-1', 'p', 'claude', 100, daysAgo(1))
    insertToolResult(testDb, 'Bash', 1, 'sess-1', 'p', 'claude', 100, daysAgo(2))
