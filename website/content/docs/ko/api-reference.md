...
특정 도구의 집계 통계, 일별 추이, 관련 세션을 반환합니다.

**경로 파라미터**

| 이름 | 타입 | 설명 |
|------|------|------|
| name | string | 도구 이름 (URL 인코딩) |
...
| 필드 | 타입 | 설명 |
|------|------|------|
| name | string | 프로젝트 이름 (최대 200자) |
| path | string | 프로젝트 디렉토리 절대 경로 (최대 500자, 존재해야 함) |
...
지정된 날짜 범위의 일별 집계 지표를 반환합니다. Usage Impact 탭에서 설정 변경 전후의 지표를 비교하는 데 사용됩니다.
...
AI 에이전트의 텔레메트리 설정을 자동으로 구성하여 Argus에 연결합니다.
...
| 필드 | 타입 | 기본값 | 설명 |
|------|------|--------|------|
| agents | string[] | **(필수)** | 연결할 에이전트 목록 (`claude`, `codex`, `gemini`) |
| endpoint | string | `"http://localhost:9845"` | OTLP 수신 엔드포인트 URL |
...
AI 에이전트의 텔레메트리 설정을 제거하여 Argus 연결을 해제합니다.
...
| 필드 | 타입 | 설명 |
|------|------|------|
| agents | string[] | 연결 해제할 에이전트 목록 (`claude`, `codex`, `gemini`) |
...
애플리케이션에 데이터가 존재하는지 확인합니다. 온보딩 UI에서 초기 상태를 결정하는 데 사용됩니다.
...
| 필드 | 타입 | 설명 |
|------|------|------|
| hasData | boolean | `agent_logs` 테이블에 레코드가 하나 이상 존재하면 `true` |
...
지정된 시간 범위의 실시간 에이전트 활동 데이터를 반환합니다. PiP(Picture-in-Picture) 플로팅 윈도우의 토큰 하트비트 차트에 사용됩니다.
